...
export const geminiTools = [
  {
    functionDeclarations: [
      ...
      {
        name: "getSavingsOverview",
        description: "Retrieve comprehensive saving management data including all saving pots, total deposits, withdrawals, and current net balances for the user.",
        parameters: {
          type: "OBJECT",
          properties: {},
        },
      },
      {
        ...
        description: "Withdraw a specific amount from a user's saving pot (e.g. Mutual Fund SIP, Emergency Fund, Gold) back into liquid cash.",
        parameters: {
          type: "OBJECT",
          properties: {
            potTitle: { type: "STRING", description: "Name/Title of the saving pot to withdraw from." },
            amount: { type: "NUMBER", description: "Amount to withdraw (must not exceed available balance)." },
            description: { type: "STRING", description: "Optional withdrawal reason or notes." },
            date: { type: "STRING", description: "Optional ISO date string (YYYY-MM-DD)." },
          },
          required: ["potTitle", "amount"],
        },
      },
      {
        name: "depositToSavings",
        description: "Deposit money into a new or existing saving pot.",
        parameters: {
          type: "OBJECT",
          properties: {
            potTitle: { type: "STRING", description: "Name of the saving pot to deposit into." },
            amount: { type: "NUMBER", description: "Amount to save/deposit." },
            description: { type: "STRING", description: "Optional notes." },
            useSalaryBalance: { type: "BOOLEAN", description: "Whether to deduct from active salary balance (default false)." },
            salaryMonth: { type: "INTEGER", description: "Optional salary month (1-12)." },
            salaryYear: { type: "INTEGER", description: "Optional salary year." },
            date: { type: "STRING", description: "Optional ISO date string (YYYY-MM-DD)." },
          },
          required: ["potTitle", "amount"],
        },
  ...
  switch (name) {
    ...
    case "getSavingsOverview": {
      ...
      for (const e of savingEntries) {
        ...
        if (!potsMap[k]) potsMap[k] = { title, deposited: 0, withdrawn: 0, balance: 0, count: 0 };
        potsMap[k].deposited += parseFloat(e.amount);
        potsMap[k].count += 1;
      }
      for (const w of withdrawalEntries) {
        let potTitle = w.title.replace('Withdrawal: ', '').trim();
        if (w.description) {
          const m = w.description.match(/Withdrawal from savings pot "([^"]+)"/i);
          if (m && m[1]) potTitle = m[1].trim();
        }
        const k = potTitle.toLowerCase();
        if (!potsMap[k]) potsMap[k] = { title: potTitle, deposited: 0, withdrawn: 0, balance: 0, count: 0 };
        potsMap[k].withdrawn += parseFloat(w.amount);
      }
      const pots = Object.values(potsMap).map(p => {
        p.balance = Math.max(0, p.deposited - p.withdrawn);
        return p;
      });
      const totalSaved = pots.reduce((sum, p) => sum + p.deposited, 0);
      const totalWithdrawn = pots.reduce((sum, p) => sum + p.withdrawn, 0);
      return {
        totalSaved,
        totalWithdrawn,
        netSavingsBalance: Math.max(0, totalSaved - totalWithdrawn),
        pots,
      };
    }

    case "withdrawFromSavings": {
      const { potTitle, amount, description, date } = args;
      if (!potTitle || !amount) throw new Error('Missing potTitle or amount');
      const parsedAmount = parseFloat(amount);
      if (isNaN(parsedAmount) || parsedAmount <= 0) throw new Error('Invalid withdrawal amount');

      const normTitle = potTitle.trim();
      const savingEntries = await db.financialEntry.findMany({
        where: { userId, type: 'SAVINGS', title: { equals: normTitle } },
      });
      const savingIds = savingEntries.map(e => e.id);
      ...
      const existingWithdrawals = await db.financialEntry.findMany({
        where: {
          ...
          OR: [
            ...(savingIds.length > 0 ? [{ parentEntryId: { in: savingIds } }] : []),
            { description: { contains: `Withdrawal from savings pot "${normTitle}"` } },
            { title: { equals: `Withdrawal: ${normTitle}` } }
          ]
        }
      });
      ...
      if (parsedAmount > available + 0.01) {
        throw new Error(`Insufficient savings balance in pot "${normTitle}". Available: ${available.toFixed(2)}, requested: ${parsedAmount}`);
      }
      ...
      const created = await db.financialEntry.create({
        data: {
          userId,
          amount: parsedAmount,
          title: `Withdrawal: ${normTitle}`,
          type: 'ADVANCE',
          description: description || `Withdrawal from savings pot "${normTitle}"`,
          date: date ? new Date(date) : new Date(),
          parentEntryId: savingEntries.length > 0 ? savingEntries[0].id : null,
          isAiGenerated: true,
        }
      });
      ...
      return {
        success: true,
        message: `Successfully withdrew ${parsedAmount} from "${normTitle}". Remaining pot balance is ${(available - parsedAmount).toFixed(2)}.`,
        withdrawal: {
          id: created.id,
          title: created.title,
          amount: parsedAmount,
          date: created.date.toISOString().split('T')[0],
          remainingPotBalance: Math.max(0, available - parsedAmount)
        }
      };
    }

    case "depositToSavings": {
      const { potTitle, amount, description, useSalaryBalance, salaryMonth, salaryYear, date } = args;
      if (!potTitle || !amount) throw new Error('Missing potTitle or amount');
      const parsedAmount = parseFloat(amount);
      if (isNaN(parsedAmount) || parsedAmount <= 0) throw new Error('Invalid deposit amount');
      ...
      const entryData = {
        ...
        title: normTitle,
        type: 'SAVINGS',
        description: description || `Deposit into savings pot "${normTitle}"`,
        date: date ? new Date(date) : now,
        useSalaryBalance: !!useSalaryBalance,
        isAiGenerated: true,
      };
      ...
      if (useSalaryBalance) {
        ...
        if (salRem + bonRem < parsedAmount) {
          throw new Error(`Insufficient salary balance in ${m}/${y}. Available: ${(salRem + bonRem).toFixed(2)}`);
        }
        let rem = parsedAmount;
        if (salRem > 0) {
          const deductSal = Math.min(rem, salRem);
      ...
      return {
        success: true,
        message: `Successfully deposited ${parsedAmount} into "${normTitle}".`,
        deposit: {
          id: created.id,
          title: created.title,
          amount: parsedAmount,
