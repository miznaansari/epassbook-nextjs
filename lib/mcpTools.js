...
export const MCP_TOOLS = [
  ...
  },
  {
    ...
    description: 'Retrieve comprehensive saving management summary, all saving pots, balances, total deposits, and withdrawals.',
    inputSchema: {
      type: 'object',
      properties: {},
    },
  },
  {
    ...
    description: 'Withdraw funds from an accumulated saving pot back into user liquid cash/ledger.',
    inputSchema: {
      type: 'object',
      properties: {
        potTitle: { type: 'string', description: 'Name of the saving pot to withdraw from.' },
        amount: { type: 'number', description: 'Amount to withdraw (must be <= available balance).' },
        description: { type: 'string', description: 'Optional withdrawal description.' },
  ...
  },
  {
    ...
    description: 'Deposit funds into a saving pot with optional active salary deduction.',
    inputSchema: {
      type: 'object',
      properties: {
        potTitle: { type: 'string', description: 'Name of the saving pot.' },
        amount: { type: 'number', description: 'Amount to deposit.' },
        useSalaryBalance: { type: 'boolean', description: 'Whether to deduct from salary balance.' },
        salaryMonth: { type: 'number', description: 'Salary month (1-12).' },
        salaryYear: { type: 'number', description: 'Salary year.' },
        description: { type: 'string', description: 'Optional deposit notes.' },
      },
      required: ['potTitle', 'amount'],
    },
  },
...
export async function executeMcpTool(name, args = {}, user) {
  ...
  switch (name) {
    ...
    case 'withdraw_from_savings': {
      ...
      const savingEntries = await db.financialEntry.findMany({
        where: { userId: user.id, type: 'SAVINGS', title: { equals: normTitle } },
      });
      const savingIds = savingEntries.map(e => e.id);
      ...
      return {
        success: true,
        message: `Successfully withdrew ${user.currency} ${parsedAmount} from "${normTitle}". Remaining pot balance: ${user.currency} ${(available - parsedAmount).toFixed(2)}.`,
        withdrawal: {
          id: created.id,
          title: created.title,
          amount: parsedAmount,
    ...
    case 'deposit_to_savings': {
      const { potTitle, amount, description, useSalaryBalance, salaryMonth, salaryYear } = args;
      if (!potTitle || !amount) throw new Error('Missing potTitle or amount');
      const parsedAmount = parseFloat(amount);
      ...
      return {
        success: true,
        message: `Successfully deposited ${user.currency} ${parsedAmount} into "${normTitle}".`,
        deposit: {
          id: created.id,
          title: created.title,
          amount: parsedAmount,
